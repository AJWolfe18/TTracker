...
(function() {
  ...
  function TabNavigation({ activeTab, tabs }) {
  ...
  function ThemePreviewApp() {
    ...
    useEffect(() => {
      if (window.FeatureFlags) {
        window.FeatureFlags.init()
          ...
          .catch(e => {
            console.warn('[App] FeatureFlags init failed, features stay off:', e);
          });
      }
    }, []);
    ...
    const tabs = useMemo(() => {
      ...
      if (flagsReady && window.FeatureFlags?.isEnabled?.('scotus')) {
        base.push({ id: 'scotus', label: 'Supreme Court', href: './?tab=scotus' });
      }
      base.push({ id: 'merch', label: 'Merchandise', href: './?tab=merch' });
      return base;
    }, [flagsReady]);

    // Resolve ?tab= param (re-runs when flags load)
    useEffect(() => {
      if (!window.TTShared) return;
      const tabParam = window.TTShared.getUrlParam('tab');
      if (!tabParam) return;

      if (tabParam === 'scotus') {
        if (!flagsReady) return; // Wait for flags before deciding
        if (window.FeatureFlags?.isEnabled?.('scotus')) {
          setActiveTab('scotus');
        } else {
          setActiveTab('stories');
        }
      } else if (tabParam === 'merch') {
        setActiveTab('merch');
      } else {
        setActiveTab('stories');
      }
    ...
    return React.createElement('div', { className: 'tt-preview-root', style: { display: 'flex', flexDirection: 'column', minHeight: '100vh' } },
      ...
      React.createElement(TabNavigation, { activeTab, tabs }),
