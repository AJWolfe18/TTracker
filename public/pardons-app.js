  ...
  function NewsletterForm({ signupPage, signupSource, isInline = false }) {
    ...
    const handleSubmit = async (e) => {
      ...
      if (window.turnstile && turnstileWidgetId.current) {
        ...
        if (!turnstileToken) { setStatus('error'); setMessage('Please complete the verification.'); return; }
      }
      if (!window.TTShared?.submitNewsletterSignup) {
        setStatus('error'); setMessage('Signup is temporarily unavailable. Please try again later.'); return;
      }
      setStatus('loading'); setMessage('');
      try {
        const result = await window.TTShared.submitNewsletterSignup({ email: email.trim(), turnstileToken, signupPage, signupSource });
        if (result.success) {
          ...
          setStatus('error'); setMessage(result.message);
          if (window.turnstile && turnstileWidgetId.current) window.turnstile.reset(turnstileWidgetId.current);
        }
      } catch (err) {
        setStatus('error'); setMessage(err?.message || 'Something went wrong. Please try again.');
        if (window.turnstile && turnstileWidgetId.current) window.turnstile.reset(turnstileWidgetId.current);
      }
    };
  ...
  function NewsletterFooter() {
    ...
    return React.createElement('footer', { className: 'tt-newsletter-footer' },
      React.createElement('div', { className: 'tt-newsletter-inner' },
        ...
        React.createElement(NewsletterForm, { signupPage: 'pardons', signupSource: 'footer' })
      )
    );
  }
  ...
  function InlineNewsletterCTA({ signupPage }) {
    ...
    useEffect(() => {
      ...
      const checkScroll = () => {
        ...
        if (scrollPercent >= 50) { hasTriggered = true; setShow(true); window.removeEventListener('scroll', checkScroll); }
      };
  ...
  function PardonsApp() {
    ...
    return React.createElement('div', { className: 'tt-preview-root', style: { display: 'flex', flexDirection: 'column', minHeight: '100vh' } },
      ...
      React.createElement('div', { style: { flex: 1 } },
        ...
        React.createElement(InlineNewsletterCTA, { signupPage: 'pardons' })
      ),
