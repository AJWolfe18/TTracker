...
### `stories_needing_enrichment(p_limit INTEGER DEFAULT 40, p_cooldown_hours INTEGER DEFAULT 12, p_max_failures INTEGER DEFAULT 10)` (function, migration 117 — ADO-584) — service_role only
The candidate pool for the **Stories Enrichment Agent** (`docs/features/stories-claude-agent/prompt-v1.md` Step 2, the 2-hourly claude.ai routine). Returns active stories with at least one `article_story` row that are either **never enriched** (`last_enriched_at IS NULL`) or **Claude-agent-enriched and grown since**: `enrichment_meta->>'source' = 'claude-agent'`, `last_enriched_at` older than the cooldown, AND at least one `article_story.matched_at > last_enriched_at`. Excludes legacy GPT output (no `source` marker), stories with no linked articles, and stories at or above `p_max_failures` attempts. Oldest-enriched first, never-enriched first of all; each row carries `new_article_count` and `pool_size` (total before `LIMIT`) plus the exact `last_enriched_at` the prompt's Step 6 concurrency guard needs. Replaced the prompt's inline `stories?...or=(last_enriched_at...)` filter, which re-enriched every Claude-enriched story older than 12 hours regardless of change — with ~15,000 active stories that never close, that rewrote July/August stories forever (September 18, 2026). `qa:agent-prompts` asserts the prompt calls this RPC and the RPC keeps the new-article rule.
