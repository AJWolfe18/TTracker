...
## Next-session prompt
```
/start-work ADO-583 + ADO-584 close-out, then routine health. Read docs/handoffs/2026-09-19-ado-583-executor-fixes-584-migration-117-v4.md, section 0 first. Work inline, no subagents. Do NOT merge PRs. (1) gh pr view 145 and 146. If either is unmerged: stop the close-out, remind Josh of the order (PROD SQL Editor: migration 117 v5 -> merge #145 -> merge #146) and go to step 4. (2) ADO-584: confirm PROD has 117 v5 (ask Josh to run: SELECT reason, COUNT(*) FROM public.stories_needing_enrichment(100000, 12, 3) GROUP BY reason; backlog was 75 on September 19). Read the first PROD Stories runs after the #145 merge (trig_0182WcUVyjF7Q5o2GWJMxbo1, list_runs + get_run_log): pool_size sane and falling, reasons present, Step 3B fetched new_article_ids, no permission_denied. Verify every AC, then Closed. (3) ADO-583: after #146 is on main the executor only runs on its schedule (:30 after 05/13/21 UTC, again an hour later) or via gh workflow run "Clustering Judge Executor". Read the first PROD Judge run after the merge (trig_01DDXZkpC9PkgTzU8wDdL9QM) for "published judge-run/prod/...", then the matching Actions run (collected -> executed -> branch deleted) and clustering_judge_log rows for that run_id with merged=true. No leftover judge-run/* or judge-rejected/* branches (git ls-remote). Verify AC, then Resolved. (4) Josh's open question, never answered: are the OTHER routines (SCOTUS, EO, Pardons, Fronts, Stories) hitting classifier write denials like the Judge did? RemoteTrigger list, then list_runs + get_run_log on each routine's last 2-3 runs, grep for permission_denied / denied. Read-only. Report per routine in one table. (5) Create two ADO stories (do not build unless Josh says so): (a) routine-silence Discord alert in the 6-hourly health check (no Judge log row in 12 hours / no Stories log row in 6); (b) move PROD secrets into a GitHub Environment restricted to main.
```
