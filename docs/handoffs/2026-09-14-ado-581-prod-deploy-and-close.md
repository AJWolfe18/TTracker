...
Session 2 shipped to TEST as ADO-582: migration 116 + `docs/features/fronts-claude-agent/prompt-v1.md` + the TEST routine `trig_01YL4HM5wLdqUseS1L33bvgp`. First TEST run judged all 87 candidates (3 assigned, 84 declined, 0 errors). Full story: `docs/handoffs/2026-09-15-ado-582-election-front-agent.md`. The 02:00 UTC PROD RSS run (AC 3/AC 4 proof for 581) had still not fired at 03:25 UTC; the 00:05 UTC run was on the pre-merge commit 692b305 and selected only 14 feeds (ids 3-18), so feeds 21 and 22 were NOT fetched - see the 582 handoff for the diagnosis to run on PROD.
