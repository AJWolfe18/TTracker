...
| Story merging | inline Tier A/B clustering | **Clustering Judge agent** (3x/day, prompt `judge-v1.2`) + admin manual merge/unmerge (ADO-537). Since September 19, 2026 (ADO-583) the agent only judges: it pushes `judge-inbox/<run_id>.json` on a `judge-run/<env>/<run_id>` branch and the **Clustering Judge Executor** GitHub Actions workflow (scheduled from `main`, never push-triggered; `scripts/clustering/process-judge-inbox.js` copies only the JSON file out of the branch, then `scripts/clustering/execute-judge-verdicts.js`) performs `merge_stories` + the log writes with the service key, because the cloud classifier denies the routine's own DB writes. Candidates carry **verdict memory** since migration 106 (ADO-539): a pair with a live verdict newer than either side's latest `article_story.matched_at` is skipped until a new article attaches — so a quiet Judge run is expected, not a fault. | — |
