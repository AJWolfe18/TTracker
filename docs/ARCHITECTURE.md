...
| Story merging | inline Tier A/B clustering | **Clustering Judge agent** (3x/day, prompt `judge-v1.2`) + admin manual merge/unmerge (ADO-537). Since September 19, 2026 (ADO-583) the agent only judges: it pushes `judge-inbox/<run_id>.json` on a `judge-run/<env>/<run_id>` branch and the **Clustering Judge Executor** GitHub Actions workflow (scheduled from `main`, never push-triggered; `scripts/clustering/process-judge-inbox.js` copies only the JSON file out of the branch, then `scripts/clustering/execute-judge-verdicts.js`) performs `merge_stories` + the log writes with the service key, because the cloud classifier denies the routine's own DB writes. The verdict-branch push itself is allowed on PROD only because the routine carries owner-set `auto_mode_environment` / `auto_mode_allow` entries (set September 19, 2026; see `docs/reference/cloud-agent-runbook.md` section 4). Candidates carry **verdict memory** since migration 106 (ADO-539): a pair with a live verdict newer than either side's latest `article_story.matched_at` is skipped until a new article attaches — so a quiet Judge run is expected, not a fault. | — |
...
- **Editorial dash guard** (ADO-580, migration 120): the prompts ban em dashes (`tone-system.json` bannedPatterns.emDash) but did not hold, so the database enforces it. A `strip_editorial_dashes` BEFORE INSERT/UPDATE trigger on stories, scotus_cases, executive_orders and pardons runs `public.strip_dashes` over every agent-written prose column (the list is in the migration): em dash → ` - `, en dash between digits → `-` (vote splits like 6-3), other en dash → ` - `. Verbatim quote fields (`evidence_quotes`, `evidence_anchors`) are left alone. Anything that writes those columns, admin edits included, gets the rewrite. Two older, narrower backstops follow their own rules and are not editorial storage: Facebook copy (`scripts/social/lib/copy.mjs` stripDashes: spaced dash → `, `) and Discord alert text (dash → `-`).
